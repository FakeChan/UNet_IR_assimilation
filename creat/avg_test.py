...
for folder in folder_names:
    ...
    if os.path.isfile(file_path):
        data_arr = np.load(file_path)   # Expected shape: (4,13)
        if sum_arr is None:
...
if file_count > 0:
    ...
    output_path = os.path.join("/share/home/lililei4/lzx/Unet/code/creat/", "avg_test.npy")
    np.save(output_path, avg_arr)
    print(f"Averaged result saved to: {output_path}")

    print("\nAverage RMSE (4 variables  13 pressure levels):")
    print(avg_arr)
else:
