...
class AMUSADatasetFromJSON(Dataset):
    def __init__(self, json_path):
        ...
        self.preassim_mean = torch.from_numpy(preassim_mean[1:3,:][:,:,None,None]).float()
        self.preassim_std  = torch.from_numpy(preassim_std[1:3,:][:,:,None,None]).float()
        self.era5_mean = torch.from_numpy(era5_mean[1:3,:][:, :,None, None]).float()
        self.era5_std  = torch.from_numpy(era5_std[1:3,: ][:, :,None, None]).float()
    ...
    def __getitem__(self, idx):
        ...
        # Select channels
        preassim_sel = preassim[1:3,:,:,:]      # (2,13, H, W)
        bt_sel    = bt[4:14]         # (10, H, W)
        era5_sel  = era5[1:3,:,:,:]         # (2,13, H, W)
        ...
        preassim_flat = preassim_norm.view(2 * 13, 721, 1440)  
        ...
        era5_flat = era5_norm.view(2 * 13, 721, 1440)
...
if __name__ == '__main__':
    ...
    print("input shape:", sample['input'].shape)      #  torch.Size([46, 720, 1440])
    print("target shape:", sample['target'].shape)    #  torch.Size([26, 720, 1440])
