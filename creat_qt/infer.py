...
model.load_state_dict(torch.load(r"/share/home/lililei4/lzx/Unet/code/model_qt/checkpoints/best_model.pth", map_location=device))
model.eval()
...
for idx, meta in enumerate(json_list):
    ...
    sample_dir = os.path.join(r"/share/home/lililei4/lzx/Unet/code/creat_qt/result/", folder_name)
    os.makedirs(sample_dir, exist_ok=True)

    
    sample = dataset[idx] 
    x = sample['input'].unsqueeze(0).to(device)      # (1,46,720,1440)
    y = sample['target'].unsqueeze(0).to(device)     # (1,26,720,1440)
    bt_mask = sample['bt_mask'].unsqueeze(0).to(device)  # (1,10,720,1440)

    with torch.no_grad():
        pred = model(x)  # (1,26,720,1440)

    
    pred = pred.view(2, 13, 720, 1440)   # (2,13,720,1440)
    y = y.view(2, 13, 720, 1440)
    ...
    sum_se = squared_error.sum(dim=(2, 3))          # (2,13)
    count_valid = spatial_mask.sum(dim=(2, 3))      # (1,1)
    rmse_model = torch.sqrt(sum_se / (count_valid + 1e-8))

    
    x_26 = x[:, :26, :, :]  # (1,26,720,1440)
    x_26 = x_26.view(2, 13, 720, 1440)
    denorm_x = x_26 * preassim_std + preassim_mean
    ...
    rmse_model_np = rmse_model.cpu().numpy()        
    rmse_baseline_np = rmse_baseline.cpu().numpy()  
    denorm_pred_np = denorm_pred.cpu().numpy()     
    denorm_y_np = denorm_y.cpu().numpy()            
